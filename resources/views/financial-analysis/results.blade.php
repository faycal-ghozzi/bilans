                ...
                @foreach ($ratios as $categorie => $categorieRatios)
                    ...
                    @foreach ($categorieRatios as $nom => $valeurs)
                        <tr class="hover:scale-105 transition-transform duration-150 ease-in-out">
                            ...
                            <td class="px-6 py-4">{{ number_format($valeurs['n_1'], 2) }}</td>
                            <td class="px-6 py-4">{{ number_format($valeurs['absolue'], 2) }}</td>
                            <td class="px-6 py-4">{{ number_format($valeurs['pourcentage'], 2) }}%</td>
                        </tr>
                    @endforeach
                @endforeach
            </tbody>
